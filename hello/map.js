...
var newArray_number = array_number.map(function (value) { return 2 * value; });
console.log(array_number); // [ 1, 2, 3]
console.log(newArray_number); // [ 2, 4, 6]
console.log("---------------------------------------");
// 文字列の配列をmapで変換する
var array_string = ["japanese", "english", "chinese"];
//.toUpperCase()で文字列を大文字に変換している
var upper_string = array_string.map(function (language) { return language.toUpperCase(); });
console.log(upper_string); // [ 'JAPANESE', 'ENGLISH', 'CHINESE' ]
console.log("---------------------------------------");
// ただの配列からオブジェクトの配列へ
var array_normal = [1, 2, 3];
var array_objects = array_normal.map(function (key) { return ({ key: key }); });
...
var names = objects_persons.map(function (objects_persons) { return objects_persons.name; });
console.log(names); // [ 'Alice', 'Bob', 'Charlie' ]
console.log("---------------------------------------");
// map() の中でインデックスを使う
var array_index = [5, 5, 5];
array_index = array_index.map(function (value, index) { return index; });
...
var result_items = items.indexOf("リンゴ");
// 配列の要素を追加する方法(push)----------------------
